...
def _save_vocab_to_txt_file(vocab:Tuple, path:str) -> None:
    ...
    print(f"Gram2Vec: Saving to {path}")
    with open (path, "w") as fout:
        for entry in vocab:
            fout.write(f"{entry}\n")
            
def generate_non_static_vocab(documents:List, n=50) -> None: 
    ...
    for name, count_func in vocabs.items():
        print(f"Gram2Vec: Generating vocab for {name}...")
